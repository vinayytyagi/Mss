...
export default function ShoppingProductDetailControls({ cartItem, onVariantChange }) {
  ...
  return (
    <div className="space-y-6">
      {loading ? (
        <div className="text-sm font-semibold text-slate-500">Loading options…</div>
      ) : variants.length ? (
        <>
          {colors.length ? (
            <div>
              <p className="text-base font-medium text-slate-800">Colour:</p>
          ...
          {materials.length ? (
            <div>
              <p className="text-base font-medium text-slate-800">Material:</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {materials.map((m) => {
